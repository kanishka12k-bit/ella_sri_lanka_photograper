...
/*---- reel card -----*/
 const N = 5;
  ...
  function stateOf(idx) {
    ...
    if (diff === 0)   return 'state-center';
    if (diff === 1)   return 'state-right1';
    if (diff === 2)   return 'state-right2';
    if (diff === N-1) return 'state-left1';
    if (diff === N-2) return 'state-left2';
    return 'state-off-right';
  }
